...
UI = {
 ...
 "months":  {"fr":["janvier","février","mars","avril","mai","juin","juillet","août","septembre","octobre","novembre","décembre"],"en":["January","February","March","April","May","June","July","August","September","October","November","December"],"es":["enero","febrero","marzo","abril","mayo","junio","julio","agosto","septiembre","octubre","noviembre","diciembre"],"it":["gennaio","febbraio","marzo","aprile","maggio","giugno","luglio","agosto","settembre","ottobre","novembre","dicembre"],"pt":["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],"de":["Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"],"ru":["январь","февраль","март","апрель","май","июнь","июль","август","сентябрь","октябрь","ноябрь","декабрь"],"ar":["يناير","فبراير","مارس","أبريل","مايو","يونيو","يوليو","أغسطس","سبتمبر","أكتوبر","نوفمبر","ديسمبر"],"zh":["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"],"ja":["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"],"ko":["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"],"hi":["जनवरी","फ़रवरी","मार्च","अप्रैल","मई","जून","जुलाई","अगस्त","सितंबर","अक्टूबर","नवंबर","दिसंबर"],"tr":["Ocak","Şubat","Mart","Nisan","Mayıs","Haziran","Temmuz","Ağustos","Eylül","Ekim","Kasım","Aralık"]},
    ...
    for lang in LANGS:
        ...
        def list_page(label, path, events, obj, path_fn):
            ...
            if lang == "fr":
                ...
                _mois = UI['months'][lang][int(TODAY[5:7]) - 1]
                title = f"{label} : {UI['luxury_events'][lang]} {_mois} {TODAY[:4]}, {UI['access'][lang].lower()} | ConstanceParis7"
            desc = f"{len(events)} {UI['events'][lang]} : {label}. {UI['verified_desc'][lang]}"
...
document.querySelectorAll('.ex').forEach(function(a){a.addEventListener('click',function(ev){ev.preventDefault();q.value=a.textContent;cherche();q.focus();});});
    ...
    # registre .radar/pages-imminentes.json (routine hebdomadaire) : une
    # entrée = une page requête, composée UNIQUEMENT de contenus déjà
    # vérifiés (fiche, Questions), compte à rebours recalculé à chaque passe.
    try:
        with open(os.path.join(_RAD, "pages-imminentes.json"), encoding="utf-8") as _f:
            IMM = json.load(_f)
    except Exception:
    ...
    q_par_slug = {q["slug"]: q for q in QST}
    for im in IMM:
        e = par_nom.get(im.get("evenement", ""))
        if not e:
            print(f"gen_pages: AVERTISSEMENT page imminente sans fiche : {im.get('evenement')}")
            continue
        try:
            _j = (date.fromisoformat(str(e.get("d1"))) - date.fromisoformat(TODAY)).days
        except (TypeError, ValueError):
            continue
        if str(e.get("d2", "")) < TODAY:
            continue  # édition passée : la page sort du sitemap d'elle-même
        compte = f"J-{_j}" if _j > 0 else "En ce moment"
        nr = note_radar(e)
        h1 = im.get("h1") or e.get("n", "")
        corps = [f"<div class=\"bc\"><a href=\"/\">Radar</a> · {esc(h1)}</div>",
                 f"<h1>{esc(h1)}</h1>",
                 f"<div class=\"meta\"><b>{esc(e.get('dt') or '')}</b></div>",
                 f"<div class=\"bc\">{esc(compte)}</div>"]
        if nr is not None:
            corps.append(f"<div class=\"bc\"><a href=\"/note.html\">{diamants(nr)} Note du radar : {nr}/100</a></div>")
        qs = [q_par_slug[s] for s in im.get("questions", []) if s in q_par_slug]
        if qs:
            qa = qs[0]
        ...
        corps.append("<h2 class=\"sub\">Se préparer</h2><div class=\"chips\">" + "".join(prep) + "</div>")
        autres = [x for x in im.get("autres", []) if par_nom.get(x)]
        if autres:
            corps.append(f"<h2 class=\"sub\">{esc(im.get('autres_titre') or 'À voir aussi')}</h2><div class=\"chips\">"
                         + "".join(f"<a href=\"{u_event(par_nom[x],'fr')}\">{esc(x.split(',')[0])}</a>" for x in autres)
                         + "</div>")
        radar = [f"<a href=\"{u_event(e,'fr')}\">La fiche complète de l'édition</a>"]
        for nom in im.get("fiches_liees", []):
            f2 = par_nom.get(nom)
            if f2:
                radar.append(f"<a href=\"{u_event(f2,'fr')}\">{esc(nom[:64])}</a>")
        corps.append("<h2 class=\"sub\">Sur le radar</h2><div class=\"chips\">" + "".join(radar) + "</div>")
        corps.append("<div class=\"chips\"><a href=\"/\">← Retour au radar</a><a href=\"/entrer.html\">Où voulez-vous entrer ?</a></div>")
        ld = {"@context": "https://schema.org", "@type": "Event", "name": e.get("n", ""),
        ...
        chemin = im["page"]
        write(chemin, page("fr", f"{im.get('titre') or h1} · ConstanceParis7",
              im.get("desc") or (e.get("dt") or ""), chemin, "".join(corps),
...
CSS = (
    ...
    "header.site{border-bottom:1px solid #26313a;padding:14px 0;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}@media(max-width:600px){header.site .edition{display:none}}html,body{max-width:100%;overflow-x:hidden}.wrap{overflow-wrap:anywhere}.fav-head{color:#d3b06a;text-decoration:none;font-size:12px;letter-spacing:.04em;border:1px solid rgba(211,176,106,.45);border-radius:999px;padding:5px 12px;white-space:nowrap}.fav-head:hover{background:rgba(211,176,106,.12)}"
